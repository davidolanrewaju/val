...
const SelectSeatCard = ({ seats, origin, destination }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedSeat.noofseats === parseInt(seats.no_of_seats)) {
      navigate('/bookings/details', { state: {selectedSeat, origin, destination} });
    } else {
          ...
          <form onSubmit={handleSubmit} className='w-100'>
            <div className='shadow text-center py-4 px-1'>
              <div className='border-top border-bottom' style={{ padding: '12px 5px', backgroundColor: 'rgba(112, 112, 112, 0.07)' }}>
                <h1 className='mb-3' style={{ fontWeight: '500', fontSize: '30px' }}>
                  {origin.name} ({origin.location}) to {destination.name} ({destination.location})
                </h1>
                <p className='m-0' style={{ color: '#28A745', fontSize: '18px' }}>
                  {seats.trip.trip_type}
                </p>
              </div>
              <div>
                ...
                <div className='m-2 text-center p-3 p-md-5'>
                  ...
                  <Button disabled={selectedSeat.noofseats !== parseInt(seats.no_of_seats)} btnName='Continue' className='btn btn-secondary my-4 py-2 px-3' style={{ backgroundColor: '#343A40' }} />
                </div>
              </div>
            </div>
          </form>
